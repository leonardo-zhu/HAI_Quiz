...
# Jaccard similarity function with lowercase and stemming
def jaccard_similarity_with_stemming(doc1, doc2):
    set1 = set(stemmer.stem(word) for word in nltk.word_tokenize(doc1.lower()))
    set2 = set(stemmer.stem(word) for word in nltk.word_tokenize(doc2.lower()))
...
similarities_with_stemming = [jaccard_similarity_with_stemming(query, doc) for doc in corpus]

# Find the highest and median similarity after stemming
max_similarity_with_stemming = max(similarities_with_stemming)
median_similarity_with_stemming = np.median(similarities_with_stemming)

# Output the results
print(f"Max similarity after stemming: {max_similarity_with_stemming}")
print(f"Median similarity after stemming: {median_similarity_with_stemming}")